...
// usersArr:Array<Iusers>=JSON.parse(localStorage.getItem('userArray') || "[]")
  ...
  getUserData(id: string): Iusers  {
    return this.usersArr.find((user) => user.userId === id) as Iusers;
  }

  addUser(userObj:Iusers){
    this.usersArr.push(userObj);
    // localStorage.setItem('userArray',JSON.stringify(this.usersArr))
    this._router.navigate(['/users'])
  }

  updateUser(updatedObj:Iusers){
    //API call to update user in DB
    let getId = this.usersArr.findIndex(user=>user.userId===updatedObj.userId);
    this.usersArr[getId]=updatedObj;
    this._router.navigate(['/users']);

  }

  removeUser(removeId:string){
    // this._dialog.openDialoge('Confirmation', 'Are you sure,you want to delete this user?')
    // .subscribe(result => {
    //     if(result){
            let removeIndex= this.usersArr.findIndex((user : Iusers)=>user.userId===removeId)
            let removeObj= this.usersArr[removeIndex]
            this.usersArr.splice(removeIndex, 1);
            // this._snackbar.showSnackbarMsg(`product 👉 ${removeObj.userName} is removed successfully!👍 `);
            this._router.navigate(['/users']);
    //     }
    // })
  }
