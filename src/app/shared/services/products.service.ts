...
export class ProductsService {
  ...
  productsArr: Array<Iproduct> = [
    {
      ...
      productImg:
        'https://expertphotography.com/wp-content/uploads/2021/01/73A9908.jpg',
    },
    {
      ...
      productImg:
        'https://fdn.gsmarena.com/imgroot/reviews/20/apple-iphone-12-pro/lifestyle/-1024w2/gsmarena_018.jpg',
    },
    {
      ...
      canReturn: 0,
      productDescription:
        'The Samsung Galaxy S21 Ultra features a 108MP quad camera with 100x Space Zoom, 12MP ultra-wide, and two 10MP telephoto lenses. It sports a 6.8-inch Dynamic AMOLED 2X display with a 120Hz refresh rate for smooth scrolling and gameplay. Powered by the Exynos 2100/Snapdragon 888 chipset, it runs on Android 12 with Samsung One UI 4.1. The Galaxy S21 Ultra supports 5G connectivity, has a 5000mAh battery, and offers up to 12GB RAM and 512GB storage.',
      productImg:
        'https://www.cnet.com/a/img/resize/3e495e5ec2d57f5893947cb6497fd9f4cf236c4e/hub/2021/01/20/5ac18449-617c-49ba-83f2-2971cb51078c/215-samsung-galaxy-s21-ultra-back.jpg?auto=webp&fit=crop&height=1200&width=1200',
    },
    {
      ...
      canReturn: 1,
      productDescription:
        'The Sony PlayStation 5 (PS5) delivers stunning graphics with a custom RDNA 2 GPU and 16GB GDDR6 RAM. It features a lightning-fast SSD with 825GB storage, supporting fast loading times and immersive gaming experiences. The PS5 supports 4K gaming at up to 120fps, ray tracing, and 3D audio with the new DualSense wireless controller for haptic feedback and adaptive triggers. It also supports streaming services like Netflix, Disney+, and Spotify.',
      productImg:
    ...
    },
    {
      ...
      canReturn: 1,
      productDescription:
        "The Nike Air Zoom Pegasus 38 is designed for runners seeking responsive cushioning and a reliable fit. It features Zoom Air units in the forefoot and heel for a responsive feel, offering optimal energy return with every stride. The shoe's mesh upper provides breathable comfort, while Flywire technology integrates with the laces for a secure fit. The Nike Air Zoom Pegasus 38 is suitable for daily training and long-distance running.",
      productImg:
        'https://lh3.googleusercontent.com/YZaUfqPDVlNnu2lEvvF2jXe_izPNOhn1242c3vymYm2V0u3taJuftr7o-NZ8N2jeNyh6kWtsemHabTkpOvLxQHfzq6P0kbllQ7SNpriGAL5cpF-Enm-p4jKDc3Wud91mA0JUQsO7',
    },
    {
      ...
      canReturn: 0,
      productDescription:
        'The Bose QuietComfort 45 Headphones feature Acoustic Noise Cancelling technology to block out ambient noise and provide clear audio. They offer a comfortable around-ear fit with plush ear cushions, ideal for extended listening sessions. The headphones include an intuitive control interface on the earcup for adjusting volume, managing calls, and activating voice assistants. They provide up to 24 hours of wireless playback and include a carrying case for portability.',
      productImg:
        'https://assets.bose.com/content/dam/Bose_DAM/Web/consumer_electronics/global/products/headphones/qc45/product_silo_images/QC45_PDP_Ecom-Gallery-W05.jpg/jcr:content/renditions/cq5dam.web.1280.1280.jpeg',
    },

    {
      ...
      productImg:
        'https://s7d5.scene7.com/is/image/FalabellaCO/sku_14913442_01?$fullchile$',
    },
    {
  ...
  updateProduct(updatedObj: Iproduct) {
    let updateIndex = this.productsArr.findIndex(
      (prod) => prod.pId === updatedObj.pId
    );
    this.productsArr[updateIndex] = updatedObj;
    this._snackbar.snackbar(
      `product ${updatedObj.pname} is Updated Successfully ! 👍`
    );
    this._router.navigate(['/products']);
  }

  removeProduct(id: string) {
    let removeIndex = this.productsArr.findIndex((prod) => prod.pId === id);
    let removeObj = this.productsArr[removeIndex];
    this.productsArr.splice(removeIndex, 1);
    this._snackbar.snackbar(
      `product ${removeObj.pname} is Removed Successfully ! 👍`
    );
    this._router.navigate(['/products']);
  }
}
